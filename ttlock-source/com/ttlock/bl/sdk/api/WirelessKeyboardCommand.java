...
//    固定数据	2	0x0D 0x0A
    ...
    public WirelessKeyboardCommand(byte commandType) {
     ...
     * @param command
     */
    public WirelessKeyboardCommand(byte[] command) {
        this.header = new byte[2];
...
//        LogUtil.d("mIsChecksumValid : " + mIsChecksumValid, DBG);
    ...
    public byte[] buildCommand() {
        byte[] commandBytes = new byte[2 + 7 + 1 + 1 + + 1 + this.length + 1];
