...
internal class BluetoothImpl private constructor() {
    private object InstanceHolder {
        private val mInstance = BluetoothImpl()
    }
     ...
     * 接收的数据
     */
    private var mReceivedDataBuffer: ByteArray
    ...
    private var psFromLock: ByteArray?
     ...
     * 不支持的情况不传该字段
     */
    private var settingValue //用于表示防撬开关、重置按键开关，反锁开关、开门方向、常开模式自动开锁开关、Wifi锁省电模式开关等设置项
            : Int? = null
...
//                }
                ...
                if (service != null) {
                    ...
                    if (mBluetoothGatt.getServices()
                            .size() > 0
                    ) mExtendedBluetoothDevice!!.isNoLockService = true
                    close()
                    //再次扫描
//                    startScan();
                    mExtendedBluetoothDevice!!.disconnectStatus =
...
//        LogUtil.d("send command:" + (char) transferData.getCommand() + "-" + String.format("%#x", transferData.getCommand()), DBG);;
    ...
    fun sendCommand(
        ...
        when (apiCommand) {
            ...
            APICommand.OP_SET_KEYBOARD_PASSWORD, APICommand.OP_ADD_ONCE_KEYBOARD_PASSWORD, APICommand.OP_ADD_PERIOD_KEYBOARD_PASSWORD, APICommand.OP_ADD_PERMANENT_KEYBOARD_PASSWORD, APICommand.OP_REMOVE_ONE_PASSWORD, APICommand.OP_MODIFY_KEYBOARD_PASSWORD -> newPwd =
                string
            else -> {}
        }
        sendCommand(commandSrc, apiCommand)
    ...
    private fun processCommandResponse(values: ByteArray) {
        mAppExecutor.mainThread().execute(Runnable {
            ...
            if (currentAPICommand == APICommand.OP_GET_LOCK_VERSION) {
                ...
                val mLockVersion = LockVersion(protocolType, protocolVersion, scene, groupId, orgId)
            ...
            addSdkLog()
            sdkLogItem = "command:" + Char(command.getCommand().toUShort()) + "-" + String.format(
                "%#x",
                command.getCommand()
            )
            addSdkLog()
            when (command.lockType) {
                LockType.LOCK_TYPE_V2, LockType.LOCK_TYPE_V2S, LockType.LOCK_TYPE_MOBI, LockType.LOCK_TYPE_CAR -> data =
                    command.getData()
            ...
            addSdkLog()
            sdkLogItem =
                "feedback comman:" + Char(data[0].toUShort()) + "-" + String.format("%#x", data[0])
            addSdkLog()
            ...
            if (command.getCommand() == Command.Companion.COMM_GET_AES_KEY) {
                if (data[1].toInt() == 1) {
                    ...
                    CommandUtil.V_addAdmin(command.lockType, adminPs, unlockKey, aesKeyArray)
                } else { //失败
                ...
                if (data[1] == CommandResponse.SUCCESS) {
                    ...
                    when (data[0]) {
                        ...
                        Command.Companion.COMM_ADD_ADMIN -> {
                            //V
                            //TODO:是否判断车位锁的
                            if (command.lockType == LockType.LOCK_TYPE_V3) {
                        ...
                        Command.Companion.COMM_SEARCH_BICYCLE_STATUS -> {
                            //自行车锁状态
                            isWaitCommand = true
                            mExtendedBluetoothDevice!!.parkStatus = data[3].toInt()
...
//                                    TTLockAPI.getTTLockCallback().onGetDoorSensorState(mExtendedBluetoothDevice, data[2], data[4], error);
                        ...
                        Command.Companion.COMM_FUNCTION_LOCK -> {
                            ...
                            val uid: Int =
                                DigitUtil.fourBytesToLong(Arrays.copyOfRange(data, 3, 7)).toInt()
                            val uniqueid: Int =
                                DigitUtil.fourBytesToLong(Arrays.copyOfRange(data, 7, 11)).toInt()
                        ...
                        Command.Companion.COMM_CHECK_ADMIN -> {
                            ...
                            when (currentAPICommand) {
                                ...
                                APICommand.OP_LOCK_ADMIN, APICommand.OP_LOCK_EKEY -> if (command.lockType == LockType.LOCK_TYPE_V3_CAR) {
                                    CommandUtil.lock(
                                        command.lockType,
                                        unlockKey,
                                        psFromLock,
                                ...
                                APICommand.OP_SET_DELETE_PASSWORD -> if (command.lockType == LockType.LOCK_TYPE_V3) //三代锁要校验随机数
                                    CommandUtil.checkRandom(
                                        command.lockType,
                                        unlockKey,
                                        psFromLock,
                                ...
                                APICommand.OP_RESET_LOCK -> if (command.lockType == LockType.LOCK_TYPE_V3 || command.lockType == LockType.LOCK_TYPE_V3_CAR) {
                                    CommandUtil.checkRandom(
                                        command.lockType,
                                        unlockKey,
                                        psFromLock,
                                    ...
                                    CommandUtil.R_resetLock(command.lockType)
                                }
                                APICommand.OP_INIT_PWD -> when (command.lockType) {
                                    ...
                                    LockType.LOCK_TYPE_V2S -> {
                                        ...
                                        val pwds = generatePwd(KeyboardPwd.Companion.ONE_DAY_PWD)
                                        CommandUtil_V2S.synPwd(command.lockType, pwds, 0)
                                    }
                                    else -> {}
                                }
                        ...
                        Command.Companion.COMM_SET_ADMIN_KEYBOARD_PWD -> if (currentAPICommand == APICommand.OP_ADD_ADMIN) { //补充管理员后续指令
                            if (command.lockType == LockType.LOCK_TYPE_V3) //三代没有删除码
...
//                                    TTLockAPI.getTTLockCallback().onSetDeletePassword(mExtendedBluetoothDevice, deletePwd, error);
                        ...
                        Command.Companion.COMM_SYN_KEYBOARD_PWD -> {
                            when (command.lockType) {
                                LockType.LOCK_TYPE_V2S_PLUS -> {
                                    ...
                                    if (dataPos + 1 < pwdData.size) {
                                        ...
                                        if (currentAPICommand == APICommand.OP_ADD_ADMIN) {
                                            ...
                                            lockData.lockVersion = command.lockVersion
                                ...
                                LockType.LOCK_TYPE_V2S -> {
                                    ...
                                    var pwdType = 0
                                    if (seq < 300) pwdType = 1 else if (seq < 450) pwdType =
                                        2 else if (seq < 550) pwdType =
                                        3 else if (seq < 650) pwdType =
                                        4 else if (seq < 700) pwdType =
                                        5 else if (seq < 750) pwdType =
                                        6 else if (seq < 800) pwdType =
                                        7 else if (seq < 900) pwdType = 8 else {
                                        try {
                                    ...
                                    CommandUtil_V2S.synPwd(command.lockType, pwds, seq)
                                }
                            }
                        }
                        Command.Companion.COMM_CHECK_RANDOM -> {
                            ...
                            when (currentAPICommand) {
                                ...
                                APICommand.OP_RESET_LOCK -> CommandUtil.R_resetLock(command.lockType)
                        ...
                        Command.Companion.COMM_MANAGE_KEYBOARD_PASSWORD -> {
                            when (data[3]) {
                                PwdOperateType.PWD_OPERATE_TYPE_ADD -> {
                                    when (currentAPICommand) {
                                        APICommand.OP_ADD_ONCE_KEYBOARD_PASSWORD -> keyboardPwdType =
                                            KeyboardPwdType.PWD_TYPE_PERIOD //返回类型换一下,同服务端一致
                                        APICommand.OP_ADD_PERIOD_KEYBOARD_PASSWORD -> keyboardPwdType =
                                            KeyboardPwdType.PWD_TYPE_COUNT //返回类型换一下,同服务端一致
                                        APICommand.OP_ADD_PERMANENT_KEYBOARD_PASSWORD -> keyboardPwdType =
                                            KeyboardPwdType.PWD_TYPE_PERMANENT
                                        else -> {}
                                    }
                        ...
                        Command.Companion.COMM_GET_LOCK_TIME -> {
                            ...
                            if (mTimeCallback != null) {
                                (mTimeCallback as GetLockTimeCallback).onGetLockTimeSuccess(calendar.getTimeInMillis())
                            }
                        }
                        Command.Companion.COMM_CHECK_USER_TIME -> {
                            ...
                            when (currentAPICommand) {
                                ...
                                APICommand.OP_CALIBRATE_TIME -> {
                                     ...
                                     * 三代锁要校验随机数
                                     */
                                    if (command.lockType == LockType.LOCK_TYPE_V3 || command.lockType == LockType.LOCK_TYPE_V3_CAR) CommandUtil.checkRandom(
                                        command.lockType,
                                        unlockKey,
                                        psFromLock,
                                        aesKeyArray
                                ...
                                APICommand.OP_ACTIVATE_FLOORS -> {
                                    ...
                                    if (mControlLockCallback != null) {
                                        (mControlLockCallback as ActivateLiftFloorsCallback).onActivateLiftFloorsSuccess(
                                            ActivateLiftFloorsResult(battery, uniqueid, lockTime)
                                        )
                                    }
                                }
                                else ->
                                     ...
                                     * 车位锁进行相反回调
                                     */
                                    if (command.lockType == LockType.LOCK_TYPE_CAR) {
                                        val mControlLockCallback: LockCallback =
                                            LockCallbackManager.Companion.getInstance()
                                                .getCallback()
                                        if (mControlLockCallback != null) {
                        ...
                        Command.Companion.COMM_GET_ALARM_ERRCORD_OR_OPERATION_FINISHED -> {
                            if (command.lockType == LockType.LOCK_TYPE_V3 || command.lockType == LockType.LOCK_TYPE_V3_CAR) { //三代锁添加完成
...
//                                    mHandler.postDelayed(disConRunable, 1500);
                                ...
                                if (len == 8) {
                                    if (data[7].toInt() == 1) { //还有后续数据要读
                                        ...
                                        CommandUtil_Va.Va_Get_Lockcar_Alarm(command.lockType)
                                    } else {
...
//                                        }
                        ...
                        Command.Companion.COMM_CONTROL_REMOTE_UNLOCK -> {
                            ...
                            when (currentAPICommand) {
                                APICommand.OP_CONTROL_REMOTE_UNLOCK -> {
                                    ...
                                    CommandUtil.searchDeviceFeature(command.lockType)
                                }
                        ...
                        Command.Companion.COMM_PWD_LIST -> {
                            val totalRecord: Int =
                                (data[2] shl 8 or (data[3] and 0xff)).toShort().toInt()
                            //无数据
                            if (totalRecord == 0) {
                                ...
                                val nextSeq = parsePasscode(Arrays.copyOfRange(data, 4, data.size))
                                if (nextSeq.toInt() == -1) {
                                    val mGetPasscodeCallback: LockCallback =
                                        LockCallbackManager.Companion.getInstance().getCallback()
                                    if (mGetPasscodeCallback != null) {
                        ...
                        Command.Companion.COMM_IC_MANAGE -> {
                            val battery = data[2].toInt()
                            when (data[3]) {
                                ICOperate.IC_SEARCH -> {
                                    LogUtil.d("ic bytes:" + Arrays.toString(data))
                                    val nextSeq = parseIC(Arrays.copyOfRange(data, 4, data.size))
                                ...
                                ICOperate.DELETE -> when (currentAPICommand) {
                                    ...
                                    APICommand.OP_LOSS_IC -> {
                                        mDeleteCard = LockCallbackManager.Companion.getInstance()
                                            .getCallback()
                                        if (mDeleteCard != null) {
                        ...
                        Command.Companion.COMM_FR_MANAGE -> {
                            battery = data[2].toInt()
                            when (data[3]) {
                                ICOperate.FR_SEARCH -> {
                                    val nextSeq = parseFR(Arrays.copyOfRange(data, 4, data.size))
                                    LogUtil.d("search:$nextSeq", DBG)
                                    if (nextSeq.toInt() == -1) {
                                        //TODO:
                                ...
                                ICOperate.WRITE_FR -> {
                                    isWaitCommand = true
                                    val seq: Short =
                                        ((data[4] shl 8 or (data[5] and 0xff)) + 1).toShort()
                                    val leftLen: Int =
...
//                                            packetLen = leftLen;
                        ...
                        Command.Companion.COMM_SET_WRIST_BAND_KEY -> battery = data[2].toInt()
                            ...
                            when (op) {
                                AutoLockOperate.SEARCH -> {
                                    ...
                                    val currentTime: Short =
                                        (data[4] shl 8 or (data[5] and 0x000000ff)).toShort()
                                    val minTime: Short =
                                        (data[6] shl 8 or (data[7] and 0x000000ff)).toShort()
                                    val maxTime: Short =
                                        (data[8] shl 8 or (data[9] and 0x000000ff)).toShort()
                        ...
                        Command.Companion.COMM_READ_DEVICE_INFO -> {
                            ...
                            when (tempOptype) {
                                DeviceInfoType.MODEL_NUMBER -> {
                                    ...
                                    modelNumber = String(Arrays.copyOfRange(data, 2, data.size - 1))
                                    if (lockData != null) {
                                ...
                                DeviceInfoType.MANUFACTURE_DATE -> {
                                    factoryDate = String(Arrays.copyOfRange(data, 2, data.size - 1))
                                    if (lockData != null) {
                                    ...
                                    if (FeatureValueUtil.isSupportFeature(
                                        ...
                                        if (currentAPICommand == APICommand.OP_ADD_ADMIN) { //三代锁
                                            if (transferData != null) mExtendedBluetoothDevice!!.remoteUnlockSwitch =
                                                transferData.getOpValue()
                                            lockData = getLockInfoObj()
                                            lockData.lockVersion = command.lockVersion
                                            CommandUtil.operateFinished(command.lockType)
...
//                                                }
                        ...
                        Command.Companion.COMM_READ_PWD_PARA -> {
                            battery = data[2].toInt()
                            val code: Int = data[3] shl 4 or (data[4] shr 4 and 0x0f) and 0x0fff
                            LogUtil.d("bytes:" + DigitUtil.byteArrayToHexString(data), DBG)
                            val secretKey =
                                data[4] * 1L shl 32 and 0x0f00000000L or (data[5] shl 24L and 0xff000000L) or (data[6] shl 16L and 0x00ff0000) or (data[7] shl 8L and 0xff00) or (data[8] and 0xff) and 0x0fffffffffL
...
//                                    }
                                ...
                                val mNbCallback: LockCallback =
                                    LockCallbackManager.Companion.getInstance().getCallback()
                                if (mNbCallback != null) {
                                    (mNbCallback as SetNBServerCallback).onSetNBServerSuccess(data[2].toInt())
                                }
                            }
                        }
                            ...
                            if (data[3].toInt() == HotelData.Companion.GET) {
                                ...
                                when (data[4]) {
                                    ...
                                    HotelData.Companion.TYPE_POWER_SAVER_CONTROLABLE_LOCK -> {
                                        callback = LockCallbackManager.Companion.getInstance()
                                            .getCallback()
                                        if (callback != null) {
                                ...
                                PassageModeOperate.QUERY -> if (data[4] == 0xff.toByte()) {
                                    ...
                                    if (mGetPassageModeCallback != null) {
                                        (mGetPassageModeCallback as GetPassageModeCallback).onGetPassageModeSuccess(
                                            GsonUtil.toJson<List<PassageModeData>>(passageModeDatas)
                                        )
                                    }
                                } else {
                                    doWithPassageModeData(Arrays.copyOfRange(data, 5, data.size))
                        ...
                        Command.Companion.COMM_LAMP -> when (data[3]) {
                            OperationType.GET_STATE -> {
                                lockData.lightingTime =
                                    data[5] and 0xff or (data[4] shl 8 and 0xff00) and 0xffff
                                doResponse(data[0], command)
                            }
                        ...
                        Command.Companion.COMM_SWITCH -> when (data[3]) {
                            OperationType.GET_STATE -> {
                                switchItem =
                                    DigitUtil.bytesToLong(Arrays.copyOfRange(data, 4, 8)).toInt()
                                switchValue =
                                    DigitUtil.bytesToLong(Arrays.copyOfRange(data, 8, 12)).toInt()
                                ...
                                when (currentAPICommand) {
                                    ...
                                    APICommand.OP_GET_UNLOCK_DIRECTION -> {
                                        lockCallback = LockCallbackManager.Companion.getInstance()
                                            .getCallback()
                        ...
                        Command.Companion.COMM_DEAD_LOCK -> {
                            ...
                            battery = data[2].toInt()
                            uid = DigitUtil.fourBytesToLong(Arrays.copyOfRange(data, 3, 7)).toInt()
                            uniqueid =
                                DigitUtil.fourBytesToLong(Arrays.copyOfRange(data, 7, 11)).toInt()
                            calendar.set(
                                2000 + data[11],
                            ...
                            calendar.setTimeZone(timeZone)
                            lockTime = calendar.getTimeInMillis()
                            mExtendedBluetoothDevice!!.batteryCapacity = battery.toByte()
                        }
                        ...
                        Command.Companion.COMM_ACCESSORY_BATTERY -> {
                            ...
                            lockCallback = LockCallbackManager.Companion.getInstance().getCallback()
            ...
            if (command.lockType == LockType.LOCK_TYPE_V3 && !isWaitCommand) {
//            LogUtil.d("start the time", true);
    ...
    fun generateTransmissionData(scene: Int, values: ByteArray, validPwdNum: Byte): String {
        //300个密码
        val pwdSet: MutableSet<*> = LinkedHashSet<String>()
    ...
    fun clearTask() {
        ...
        if (mHandler != null) mHandler.removeCallbacks(disConRunable)
        if (disTimerTask != null) disTimerTask.cancel()
        ...
        if (null != gatt) {
            try {
                ...
                val localMethod: Method =
                    localBluetoothGatt.getClass().getMethod("refresh", arrayOfNulls<Class<*>>(0))
                if (localMethod != null) {
                    return (localMethod.invoke(
                        localBluetoothGatt, arrayOfNulls<Any>(0)
                    ) as Boolean).toBoolean()
                }
    ...
    fun parseIC(datas: ByteArray): Short {
        val nextReq: Short = (datas[0] shl 8 or (datas[1] and 0xff)).toShort()
        var dataIndex = 2
    ...
    private fun parsePasscode(datas: ByteArray): Short {
        ...
        while (dataIndex < datas.size) {
            ...
            when (passcode.keyboardPwdType) {
                ...
                4 -> passcode.cycleType =
                    (datas[dataIndex++] shl 8 or (datas[dataIndex++] and 0xff)).toShort().toInt()
                else -> {}
            }
...
//        }
    ...
    private fun initLock(command: Command) {
        if (command.lockType == LockType.LOCK_TYPE_V3_CAR || command.lockType == LockType.LOCK_TYPE_V3) {
            CommandUtil.searchDeviceFeature(command.lockType)
            //车位锁没有后续指令
        } else if (command.lockType == LockType.LOCK_TYPE_CAR) {
            val lockData: LockData? = getLockInfoObj()
            lockData.lockVersion = command.lockVersion
            lockData.noKeyPwd = ""
            lockData.deletePwd = ""
            ...
            //接后续指令 全部完成再回调
            adminPasscode = DigitUtil.generatePwdByLength(7)
            CommandUtil.S_setAdminKeyboardPwd(command.lockType, adminPasscode, aesKeyArray)
        }
    }
    ...
    private fun addFRResponse(command: Command, data: ByteArray) {
        if (data[4].toInt() == 0x01) { //添加成功
            ...
            isWaitCommand = true
            val seq: Short = (data[5] shl 8 or (data[6] and 0xff)).toShort()
            packetLen = (data[7] shl 8 or (data[8] and 0xff)).toShort().toInt()
            CommandUtil_V3.writeFR(
                command,
                transferData.getTransferData(),
                seq,
    ...
    private fun modifyFrPeriodResponse(command: Command) {
        when (currentAPICommand) {
            APICommand.OP_ADD_FR -> when (transferData.getValidityInfo().getModeType()) {
                ...
                ValidityInfo.Companion.CYCLIC -> {
                    ...
                    if (cyclicConfigs != null && cyclicPos < cyclicConfigs!!.size) {
                        ...
                        mAddFingerprint = LockCallbackManager.Companion.getInstance().getCallback()
                        if (mAddFingerprint != null) {
                            (mAddFingerprint as AddFingerprintCallback).onAddFingerpintFinished(
                                attachmentNum
                            )
            ...
            APICommand.OP_MODIFY_FR_PERIOD -> when (transferData.getValidityInfo().getModeType()) {
                ValidityInfo.Companion.TIMED -> {
    ...
    private fun errorResponse(command: Command, data: ByteArray) {
        ...
        if (data[2].toInt() == LockError.COMMAND_RECEIVED.getIntErrorCode()) { //设置wifi AP的时候表示命令接收成功，正在处理
            ...
            failNeedDelete = false
            //删除IC卡
            CommandUtil.deleteICCard(command.lockType, attachmentNum.toString(), aesKeyArray)
        } else if (currentAPICommand == APICommand.OP_ADD_FR && failNeedDelete) {
            //删除指纹
            tmpLockError = lockError
            failNeedDelete = false
            CommandUtil.deleteFR(command.lockType, attachmentNum, aesKeyArray)
...
//                || FeatureValueUtil.isSupportFeature(lockData, FeatureValue.PRIVACY_LOCK)) {
        ...
        } else if (command.lockType == LockType.LOCK_TYPE_V3_CAR) {
        ...
        } else if (command.lockType == LockType.LOCK_TYPE_V3) { //剩下三代锁走设置管理码
    ...
    private fun readModelNumber(command: Command) {
        //读取版本信息
        tempOptype = DeviceInfoType.MODEL_NUMBER.toInt()
        CommandUtil.readDeviceInfo(command.lockType, DeviceInfoType.MODEL_NUMBER, aesKeyArray)
        mExtendedBluetoothDevice!!.disconnectStatus =
            ExtendedBluetoothDevice.Companion.RESPONSE_TIME_OUT
        mHandler.postDelayed(disConRunable, 1500)
    }
    ...
    companion object {
         ...
         */
        var unlockPwdBytes: ByteArray

        /**
         * 遥控设备要用 同一次连接多次发送值相同
         */
        var uniqueidBytes: ByteArray
